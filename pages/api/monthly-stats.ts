...
export default async function handler(
  ...
  try {
    const backendUrl = `${API.BASE_URL}/api/monthly-stats?org=${encodeURIComponent(String(org))}&repo=${encodeURIComponent(String(repo))}`;
    console.log('Proxying monthly-stats request to:', backendUrl);
    ...
    return res.status(500).json({ 
      error: 'Failed to fetch monthly stats from backend',
