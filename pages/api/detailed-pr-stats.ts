...
export default async function handler(
  ...
  try {
    const backendUrl = `${API.BASE_URL}${API.ENDPOINTS.PR_STATS}?org=${encodeURIComponent(String(org))}&repo=${encodeURIComponent(String(repo))}`;
    console.log('Proxying detailed-pr-stats request to:', backendUrl);
    ...
    return res.status(500).json({ 
      error: 'Failed to fetch detailed PR stats from backend',
