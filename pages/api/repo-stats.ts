...
export default async function handler(
  ...
  try {
    const backendUrl = `${API.BASE_URL}${API.ENDPOINTS.REPO_STATS}?org=${encodeURIComponent(String(org))}&repo=${encodeURIComponent(String(repo))}`;
    console.log('Proxying repo-stats request to:', backendUrl);
    ...
    return res.status(500).json({ 
      error: 'Failed to fetch repo stats from backend',
