...
export default async function handler(
  ...
  try {
    const backendUrl = `${API.BASE_URL}${API.ENDPOINTS.ISSUE_STATS}?org=${encodeURIComponent(String(org))}&repo=${encodeURIComponent(String(repo))}`;
    console.log('Proxying issue-stats request to:', backendUrl);
    ...
    return res.status(500).json({ 
      error: 'Failed to fetch issue stats from backend',
